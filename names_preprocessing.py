...
if reload:
    ...
    totalnames_table.to_pickle('./data/Total_soc_cards.pkl')
else:
...
# Cache
names_df_trim.to_pickle('./data/names_df_trim.pkl')
...
# flawed, such as bias from which states were reporting at the time, class, immigration
# recency affecting likelihood of registration, urban/rural differences, etc., but the
# distribution of the most popular baby names before and after the big registration
# spike in the 1910s is pretty similar, so there's a good chance it doesn't make a
# *huge* difference at least.
#
...
# which will cover people born 1880 to the current day. The number of living people
# over 90 has historically been small, so we'll have a fairly complete population
...
# Store out the key variables

np.save('./data/namelife_F_full.npy',namelife_F_full)
np.save('./data/namelife_F_base.npy',namelife_F_base)
np.save('./data/namebirth_F.npy',namebirth_F)
namelife_F_name.to_pickle('./data/namelife_F_name.pkl')

np.save('./data/namelife_M_full.npy',namelife_M_full)
np.save('./data/namelife_M_base.npy',namelife_M_base)
np.save('./data/namebirth_M.npy',namebirth_M)
namelife_M_name.to_pickle('./data/namelife_M_name.pkl')